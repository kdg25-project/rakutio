...
export function errorResponse(error: unknown) {
  if (error instanceof LedgerError) return Response.json({ error: { code: error.code, message: error.message, details: error.details } }, { status: error.status })
  console.error('Ledger API failed', error instanceof LedgerError ? error.code : 'unexpected')
