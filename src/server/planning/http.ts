...
import { PlanningError, PlanningService } from './service'

export async function requirePlanning(request: Request) {
  const session = await auth.api.getSession({ headers: request.headers })
  if (!session?.user) return Response.json({ error: { code: 'UNAUTHENTICATED', message: 'ログインが必要です。' } }, { status: 401 })
  return new PlanningService(env.DB, session.user.id)
}
...
export function errorResponse(error: unknown) {
  if (error instanceof PlanningError) return Response.json({ error: { code: error.code, message: error.message, details: error.details } }, { status: error.status })
  console.error('Planning API failed', error)
