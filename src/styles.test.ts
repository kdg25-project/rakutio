...
describe('mobile-frame layout', () => {
  ...
  it('uses a bottom sheet for capture and a single-scroller full screen for review', () => {
    expect(styles).toContain('.app-receipt-capture-sheet { height: min(89dvh, 779px);')
    ...
    expect(styles).toContain(".app-receipt-screen > .receipt-flow[data-phase='review'] { align-content: start; display: flex; flex-direction: column; gap: 10px; }")
    expect(styles).toContain('.app-receipt-screen .receipt-preview { align-self: start; aspect-ratio: 4 / 5; background: #a87654; border-radius: 9px; flex: 0 0 auto; max-height: min(54dvh, 470px); }')
    ...
    expect(styles).toContain('.app-receipt-screen .receipt-pages > div > button img { object-fit: cover; }')
    expect(styles).toContain('.receipt-capture-return { align-items: center; background: transparent; border: 0;')
    ...
    expect(styles).toContain('animation: receipt-analysis-pulse 1.25s ease-in-out infinite;')
    expect(styles).not.toContain('li:nth-child(-n+2)::before')
    expect(styles).not.toContain("list-style: '✓  '")
    expect(receiptCameraStyles).toContain('.receipt-camera-live { align-self: start; background: #fff; display: flex; flex: 0 0 auto; flex-direction: column; gap: 10px; min-height: calc(min(404px, calc((100vw - 32px) / .91)) + 100px); }')
    expect(receiptCameraStyles).toContain('.receipt-camera-live__preview { aspect-ratio: auto; background: #9c684a; border-radius: 9px; flex: 0 0 auto; height: min(404px, calc((100vw - 32px) / .91));')
    expect(receiptCameraStyles).toContain('.receipt-camera-live__controls { align-items: center; display: grid; flex: 0 0 90px;')
    expect(receiptCameraStyles).toContain('@media (max-height: 680px) { .receipt-camera-live { min-height: calc(min(320px, calc((100vw - 32px) / .91)) + 78px); }')
