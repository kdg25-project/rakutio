...
export function ReceiptCamera({ onCapture, onCancel, onFallbackFiles }: ReceiptCameraProps) {
  ...
  return <section className="receipt-camera-live" aria-label="レシートを撮影">
    ...
    <div className="receipt-camera-live__controls">
      <label className="receipt-camera-live__utility"><svg className="receipt-camera-live__utility-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="3.5" y="4" width="17" height="16" rx="3" /><circle cx="9" cy="9.5" r="2" /><path d="m5.5 17 4.25-4.25 3 3 2.25-2.25L18.5 17" /></svg><span>写真を選択</span><input type="file" accept={receiptFileAccept} multiple onChange={chooseFiles} /></label>
      ...
      {fallback ? <button type="button" className="receipt-camera-live__retry" onClick={() => void startCamera(facingMode)}>カメラを再試行</button> : <button className="receipt-camera-live__utility" type="button" onClick={() => void toggleTorch()} disabled={!torchAvailable} aria-label={torchEnabled ? 'フラッシュをオフにする' : 'フラッシュをオンにする'}><svg className="receipt-camera-live__utility-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m13.25 2.5-7 10h5l-.75 9 7.25-10.5h-5.25l.75-8.5Z" /><path d="M3 21 21 3" /></svg><span>{torchEnabled ? 'オン' : 'オフ'}</span></button>}
    </div>
