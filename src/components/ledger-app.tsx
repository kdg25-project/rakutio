...
function ReceiptFlow({ onUseReceipt, onCancel }: { onUseReceipt: (receipt: { receiptId: string; extraction?: ReceiptExtraction }) => void; onCancel: () => void }) {
  ...
  return <BottomSheet label="レシートを撮影" onClose={goBack}><section className="receipt-flow screen-stack">
    ...
    {phase === 'capture' && <><ReceiptCamera onCapture={(file) => addFiles([file], false)} onFallbackFiles={addFiles} onCancel={() => { if (files.length) setPhase('review'); else onCancel() }} />{files.length > 0 && <button type="button" className="receipt-camera-review" onClick={() => setPhase('review')}>撮影した画像を確認（{files.length}枚）</button>}<p className="receipt-tip"><b>きれいに読み取るコツ</b><br />・明るい場所で撮影してください<br />・レシート全体が枠に収まるようにしてください<br />・長いレシートは複数回に分けて撮影できます</p></>}
...
function ProfileScreen({ profile, onBack, onUpdateProfile }: { profile: Profile; onBack: () => void; onUpdateProfile: (input: Partial<Profile>) => Promise<void> }) {
  const [name, setName] = useState(profile.name); const [memo, setMemo] = useState(profile.memo); const [theme, setTheme] = useState<ProfileTheme>(profile.theme); const [image, setImage] = useState<string | null>(profile.image); const [saving, setSaving] = useState(false); const [error, setError] = useState<string>()
  useEffect(() => { setName(profile.name); setMemo(profile.memo); setTheme(profile.theme); setImage(profile.image) }, [profile])
