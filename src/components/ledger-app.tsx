...
function BottomSheet({ label, onClose, children, mode = 'sheet' }: { label: string; onClose: () => void; children: ReactNode; mode?: 'sheet' | 'fullscreen' | 'receipt' | 'receipt-capture' }) {
  const ref = useRef<HTMLDialogElement>(null)
  useEffect(() => {
    const dialog = ref.current
    if (!dialog) return
  ...
  const className = mode === 'fullscreen'
    ? 'app-fullscreen-dialog'
    : mode === 'receipt'
      ? 'app-receipt-screen'
      : mode === 'receipt-capture'
        ? 'app-bottom-sheet app-receipt-capture-sheet'
        : 'app-bottom-sheet'
...
export function incomeTransactionItems(title: string, categoryId: string, amount: number): LedgerTransactionInput['items'] { return [{ categoryId, name: title.trim(), originalAmount: amount }] }

type ReceiptDraftItem = Pick<LedgerTransactionInput['items'][number], 'name' | 'originalAmount' | 'categoryId'> & { discountAmount: number; utilityKind?: UtilityKind }

function usableReceiptTotal(value: number | null | undefined): number | null {
  return typeof value === 'number' && Number.isSafeInteger(value) && value > 0 ? value : null
}

/**
 * OCR line prices are optional.  The receipt total is the authoritative value,
 * so conflicting line prices are discarded instead of allowing them to make a
 * saved transaction differ from the printed total.
 */
export function receiptDraftItems(extraction: ReceiptExtraction, categories: readonly LedgerCategory[]): ReceiptDraftItem[] {
  const items = extraction.items
    .filter((item) => item.name.trim())
    .map((item) => ({ name: item.name, originalAmount: typeof item.amount === 'number' && Number.isSafeInteger(item.amount) && item.amount >= 0 ? item.amount : 0, discountAmount: 0, categoryId: receiptItemCategoryId(categories, extraction.merchant, item.name, item.categoryId) }))
  const total = usableReceiptTotal(extraction.total)
  const lineTotal = items.reduce((sum, item) => sum + item.originalAmount, 0)
  return total !== null && lineTotal > total ? items.map((item) => ({ ...item, originalAmount: 0 })) : items
}

/** Adds the unallocated part as a transparent bookkeeping line so persisted totals always equal the receipt total. */
export function receiptItemsWithAuthoritativeTotal(items: readonly EditorItem[], receiptTotal: number | null, categoryId: string): { items: EditorItem[]; error?: string } {
  if (receiptTotal === null) return { items: [...items] }
  const lineTotal = items.reduce((sum, item) => sum + item.originalAmount, 0)
  if (lineTotal > receiptTotal) return { items: [...items], error: '各商品の金額がレシートの合計金額を超えています。' }
  // Document AI's receipt total is already the charged amount.  Line discounts
  // are not reliably extracted, so they must not reduce that authoritative value.
  const normalizedItems = items.map((item) => ({ ...item, discountAmount: 0 }))
  if (lineTotal === receiptTotal) return { items: normalizedItems }
  return {
    items: [...normalizedItems, {
      name: '未配分（レシート合計）',
      originalAmount: receiptTotal - lineTotal,
      discountAmount: 0,
      categoryId,
    }],
  }
...
function TransactionEditor({ categories, accounts, transaction, receipt, initialType, onCancel, onSave }: { categories: LedgerCategory[]; accounts: AssetAccount[]; transaction?: LedgerTransaction; receipt?: { receiptId: string; extraction?: ReceiptExtraction }; initialType: 'expense' | 'income'; onCancel: () => void; onSave: (input: LedgerTransactionInput, revision?: number, idempotencyKey?: string) => Promise<void> }) {
  ...
  const [items, setItems] = useState<EditorItem[]>(() => transaction?.items.map((item) => ({ id: item.id, name: item.name, originalAmount: item.originalAmount, discountAmount: item.itemDiscountAmount, categoryId: item.categoryId, utilityKind: item.utilityKind ?? undefined })) ?? (extraction ? receiptDraftItems(extraction, categories) : [{ name: '', originalAmount: 0, discountAmount: 0, categoryId: fallbackCategory }]))
  ...
  const [stage, setStage] = useState<'basic' | 'review'>(transaction?.type === 'income' ? 'basic' : transaction || receipt ? 'review' : 'basic')
  ...
  const receiptTotal = receipt && extraction ? usableReceiptTotal(extraction.total) : null
  const hasAuthoritativeReceiptTotal = receiptTotal !== null && type === 'expense'
  const receiptCategoryId = extraction ? receiptCategoryForExtraction(categories, extraction)?.id ?? fallbackCategory : fallbackCategory
  const preparedReceiptItems = useMemo(() => receiptItemsWithAuthoritativeTotal(items, receiptTotal, receiptCategoryId), [items, receiptTotal, receiptCategoryId])
  const calculatedItems = hasAuthoritativeReceiptTotal ? preparedReceiptItems.items : items
  const effectiveReceiptDiscountAmount = hasAuthoritativeReceiptTotal ? 0 : receiptDiscountAmount
  const effectivePointUsedAmount = hasAuthoritativeReceiptTotal ? 0 : pointUsedAmount
  const effectiveGiftCertificateUsedAmount = hasAuthoritativeReceiptTotal ? 0 : giftCertificateUsedAmount
  const calculation = useMemo(() => { try { return calculateTransaction({ items: calculatedItems, receiptDiscountAmount: effectiveReceiptDiscountAmount, pointUsedAmount: effectivePointUsedAmount, giftCertificateUsedAmount: effectiveGiftCertificateUsedAmount }) } catch (error) { return error instanceof MoneyCalculationError ? error : undefined } }, [calculatedItems, effectiveReceiptDiscountAmount, effectivePointUsedAmount, effectiveGiftCertificateUsedAmount])
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (type === 'expense' && preparedReceiptItems.error) { setInlineError(preparedReceiptItems.error); return }; const submittedItems = type === 'income' ? incomeTransactionItems(title, selectedIncomeCategoryId, Number(incomeAmount)) : calculatedItems; let submittedCalculation = calculation; if (type === 'income') { try { submittedCalculation = calculateTransaction({ items: submittedItems.map((item) => ({ ...item, discountAmount: item.discountAmount ?? 0 })), receiptDiscountAmount: 0, pointUsedAmount: 0, giftCertificateUsedAmount: 0 }) } catch (error) { submittedCalculation = error instanceof MoneyCalculationError ? error : undefined } } if (submittedCalculation instanceof Error || !submittedCalculation) { setInlineError(submittedCalculation instanceof Error ? submittedCalculation.message : '入力内容を確認してください。'); return }; if (!title.trim()) { setInlineError('タイトルを入力してください。'); return }; setInlineError(undefined); setSaving(true); try { const input: LedgerTransactionInput = type === 'income' ? { accountId: accountId || null, type, occurredAt, title, merchant, memo, paymentMethod, items: submittedItems } : { receiptId: receipt?.receiptId ?? transaction?.receiptId, accountId: accountId || null, giftAccountId: giftAccountId || null, type, occurredAt, title, merchant, memo, paymentMethod, receiptDiscountAmount: effectiveReceiptDiscountAmount, pointUsedAmount: effectivePointUsedAmount, giftCertificateUsedAmount: effectiveGiftCertificateUsedAmount, items: submittedItems }; await onSave(input, transaction?.revision, idempotencyKey.current) } finally { setSaving(false) } }

  return <form className={`entry-form screen-stack entry-${stage}`} onSubmit={stage === 'basic' && type !== 'income' ? goToReview : submit}>
    ...
    {receipt && <p className="form-note">{receipt.extraction ? receiptTotal !== null ? 'OCR結果は下書きです。各商品の金額は任意で、レシートの合計金額を正として登録します。' : 'OCR結果は下書きです。内容を確認・編集してから登録してください。' : '画像は保存されています。内容を手入力してから登録してください。'}</p>}
    {stage === 'basic' ? <>
      {!receipt && <div className="segmented"><button type="button" className={type === 'expense' ? 'active expense' : ''} onClick={() => setType('expense')}>支出</button><button type="button" className={type === 'income' ? 'active income' : ''} onClick={() => { setType('income'); if (!incomeCategories.some((category) => category.id === incomeCategoryId)) setIncomeCategoryId(fallbackIncomeCategory) }}>収入</button></div>}
      <section className="entry-basic-fields"><label>{type === 'expense' ? '店舗名' : '内容'}<input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder={type === 'expense' ? '例）カフェ' : '例）給与'} /></label><label>日付<input type="date" value={occurredAt} onChange={(event) => setOccurredAt(event.target.value)} required /></label>{type === 'income' ? <><label>収入カテゴリ<select value={selectedIncomeCategoryId} onChange={(event) => setIncomeCategoryId(event.target.value)}>{incomeCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>金額<input value={incomeAmount} onChange={(event) => setIncomeAmount(event.target.value)} type="number" min="1" step="1" inputMode="numeric" required placeholder="例）250000" /></label></> : <label>カテゴリ<select value={items[0]?.categoryId ?? fallbackCategory} onChange={(event) => setItems((previous) => previous.map((item, index) => index === 0 ? { ...item, categoryId: event.target.value } : item))}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>}<label>{type === 'income' ? '入金方法' : '支払い方法'}<input value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} placeholder={type === 'income' ? '現金・振込など' : '現金・カードなど'} /></label>{type === 'income' && <label>入金先口座（任意）<select value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">指定しない</option>{accounts.filter((account) => account.type !== 'gift').map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>}<label>メモ（任意）<textarea value={memo} onChange={(event) => setMemo(event.target.value)} placeholder={type === 'income' ? '例）9月分' : '例）日用品のまとめ買い'} /></label></section>
      {inlineError && <p className="inline-error" role="alert">{inlineError}</p>}
      <button className={`primary-action entry-fixed-action ${type === 'expense' ? 'expense-action' : ''}`} type="submit">{type === 'income' ? transaction ? '変更を保存' : '登録する' : '次へ'}</button>
    </> : <>
...
export function hasCapturedReceiptPages(files: readonly File[]) { return files.length > 0 }
...
function CategoriesScreen({ categories, onBack, onChanged, notify, confirm }: { categories: LedgerCategory[]; onBack: () => void; onChanged: () => Promise<void>; notify: (value: Notice) => void; confirm: (title: string, text: string, action: () => Promise<void>) => void }) {
