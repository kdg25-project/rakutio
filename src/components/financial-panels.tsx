...
import type { AssetAccount, AssetBalanceHistory, AssetBalanceHistoryPoint, AssetEntry, AssetSummary } from '../server/assets/types'
...
type PanelProps = { notify?: (notice: FinanceNotice) => void; onChanged?: () => Promise<void> | void }
...
function Loading({ text = '読み込み中…' }: { text?: string }) { return <div className="finance-loading"><span />{text}</div> }
...
function shortDate(value: string) { return new Intl.DateTimeFormat('ja-JP', { month: 'numeric', day: 'numeric' }).format(new Date(`${value}T00:00:00`)) }

export function AssetsScreen({ notify, onChanged }: PanelProps) {
  const { notice, show } = useNotice(notify)
  const [assets, setAssets] = useState<AssetSummary>()
  const [history, setHistory] = useState<AssetBalanceHistoryPoint[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string>()
  const [panel, setPanel] = useState<'create' | 'transfer' | 'adjust' | 'entries' | 'edit'>()
  ...
  const range = useMemo(() => assetHistoryRange(), [])
  const load = useCallback(async () => {
    setLoading(true); setError(undefined)
    try {
      const [accountResponse, historyResponse] = await Promise.all([
        api<{ assets: AssetSummary }>('/api/assets/accounts/?includeArchived=true'),
        api<AssetBalanceHistory>(`/api/assets/history?from=${range.from}&to=${range.to}`),
      ])
      setAssets(accountResponse.assets); setHistory(historyResponse.history)
    } catch (cause) { setError(cause instanceof Error ? cause.message : '資産を読み込めませんでした。') } finally { setLoading(false) }
  }, [range.from, range.to])
  useEffect(() => { void load() }, [load])
  const changed = useCallback(async (message: string) => { await load(); await onChanged?.(); show({ kind: 'success', text: message }) }, [load, onChanged, show])
  ...
  const openEntries = async (account: AssetAccount) => {
    ...
    setSelected(account); setPanel('entries'); setEntries([]); setEntriesLoading(true)
    try {
  ...
  const peak = Math.max(1, ...history.map((point) => Math.abs(point.balanceAmount)))
  if (loading) return <Loading text="資産を読み込んでいます…" />
  return <section className="finance-screen"><header className="finance-title-row"><div><p className="finance-kicker">資産</p><h1>総資産</h1></div><button type="button" className="finance-icon-button" onClick={() => setPanel('create')}><span>＋</span><small>口座を追加</small></button></header>
    ...
    <div className="finance-total-card"><span>利用中の総資産</span><strong>{yen(assets?.activeTotalAmount ?? 0)}</strong>{assets && assets.totalAmount !== assets.activeTotalAmount && <small>アーカイブを含む: {yen(assets.totalAmount)}</small>}</div>
    <section className="finance-card"><div className="finance-section-head"><h2>資産の推移</h2><span>直近6か月</span></div>{history.length ? <div className="finance-trend" aria-label="資産推移">{history.map((point) => <div key={point.date} className="finance-trend-point"><i style={{ height: `${Math.max(5, Math.round(Math.abs(point.balanceAmount) / peak * 100))}%` }} /><span>{shortDate(point.date)}</span></div>)}</div> : <p className="finance-empty-inline">この期間の残高変動はまだありません。</p>}</section>
    <section className="finance-card finance-account-card"><div className="finance-section-head"><h2>資産の内訳</h2><button type="button" onClick={() => setPanel('transfer')}>振替・商品券購入</button></div>{assets?.accounts.length ? <ul className="finance-list">{assets.accounts.map((account) => <li key={account.id} className={account.isArchived ? 'finance-muted-row' : ''}><button type="button" className="finance-account-row" onClick={() => void openEntries(account)}><span className={`finance-account-symbol finance-account-${account.type}`}><Icon name={account.type === 'bank' ? 'analytics' : account.type === 'cash' ? 'home' : 'category-subscription'} /></span><span><b>{account.name}</b><small>{kindLabel[account.type]}{account.isArchived ? '・アーカイブ済み' : ''}</small></span><strong>{yen(account.balanceAmount)}</strong><span aria-hidden="true">›</span></button><div className="finance-row-actions">{account.isArchived ? <button type="button" onClick={() => setConfirmRestore(account)}>復元</button> : <><button type="button" onClick={() => { setSelected(account); setPanel('adjust') }}>調整</button><button type="button" onClick={() => { setSelected(account); setPanel('edit') }}>編集</button></>}</div></li>)}</ul> : <div className="finance-empty"><Icon name="analytics" /><p>銀行口座・現金・商品券を追加すると、総資産を確認できます。</p><button type="button" className="finance-button" onClick={() => setPanel('create')}>最初の資産を追加</button></div>}</section>
...
export function MonthTargetScreen({ month, notify, onChanged }: PanelProps & { month: string }) {
  const { notice, show } = useNotice(notify); const [target, setTarget] = useState<MonthlyTarget>(); const [loading, setLoading] = useState(true); const [error, setError] = useState<string>(); const [editing, setEditing] = useState(false); const [busy, setBusy] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false); const retryRef = useRef<(() => Promise<void>) | undefined>(undefined)
  const load = useCallback(async () => { setLoading(true); setError(undefined); try { setTarget((await api<{ targets: MonthlyTarget[] }>(`/api/planning/monthly-targets/?month=${encodeURIComponent(month)}`)).targets[0]) } catch (cause) { setError(cause instanceof Error ? cause.message : '予算・目標を読み込めませんでした。') } finally { setLoading(false) } }, [month])
  useEffect(() => { void load() }, [load])
  const mutate = async (work: () => Promise<void>, message: string) => { setBusy(true); try { await work(); retryRef.current = undefined; await load(); await onChanged?.(); show({ kind: 'success', text: message }); setEditing(false); setConfirmDelete(false) } catch (cause) { show({ kind: 'error', text: cause instanceof Error ? cause.message : '保存できませんでした。' }); retryRef.current = () => mutate(work, message) } finally { setBusy(false) } }
  if (loading) return <Loading text="予算・目標を読み込んでいます…" />
...
export function RecurringScreen({ notify, onChanged }: PanelProps) {
  const { notice, show } = useNotice(notify); const [rules, setRules] = useState<RecurringRule[]>([]); const [categories, setCategories] = useState<LedgerCategory[]>([]); const [accounts, setAccounts] = useState<AssetAccount[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState<string>(); const [editing, setEditing] = useState<RecurringRule | 'new'>(); const [confirmStop, setConfirmStop] = useState<RecurringRule>(); const [runResult, setRunResult] = useState<RecurringRunResult>(); const [busy, setBusy] = useState(false); const retryRef = useRef<(() => Promise<void>) | undefined>(undefined)
  const load = useCallback(async () => { setLoading(true); setError(undefined); try { const [ruleResponse, categoryResponse, assetResponse] = await Promise.all([api<{ rules: RecurringRule[] }>('/api/planning/recurring-rules/'), api<{ categories: LedgerCategory[] }>('/api/ledger/categories/'), api<{ assets: AssetSummary }>('/api/assets/accounts/?includeArchived=false')]); setRules(ruleResponse.rules); setCategories(categoryResponse.categories); setAccounts(assetResponse.assets.accounts.filter((account) => account.type !== 'gift')) } catch (cause) { setError(cause instanceof Error ? cause.message : '定期支出を読み込めませんでした。') } finally { setLoading(false) } }, [])
  useEffect(() => { void load() }, [load])
  ...
  if (loading) return <Loading text="定期支出を読み込んでいます…" />
