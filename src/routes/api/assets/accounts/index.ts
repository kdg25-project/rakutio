...
export const Route = createFileRoute('/api/assets/accounts/')({
  server: { handlers: {
    ...
    POST: async ({ request }) => {
      ...
      try { const result = await assets.createAccount({ type: body.type, name: body.name, bankKind: body.bankKind, bankMemo: body.bankMemo, initialBalanceAmount: body.initialBalanceAmount, idempotencyKey: body.idempotencyKey }); return Response.json(result, { status: result.idempotent ? 200 : 201 }) } catch (error) { return errorResponse(error) }
    },
  } },
})
